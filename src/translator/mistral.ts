...
export async function translateBatchMistral(
  input: string[],
  llm: LlmConfig,
  options: TranslateOptions
): Promise<string[]> {
  const apiKeyEnv = llm.apiKeyEnv ?? "MISTRAL_API_KEY";
  const apiKey = process.env[apiKeyEnv];
  if (!apiKey) {
    throw new Error(`Missing Mistral API key in env: ${apiKeyEnv}`);
  }

  const url = llm.baseUrl ?? "https://api.mistral.ai/v1/chat/completions";
  ...
  if (!response.ok) {
    ...
    throw new Error(`Mistral request failed: ${response.status} ${response.statusText} - ${text}`);
  }
  ...
  if (parsed.length !== input.length) {
    throw new Error(
      `Mistral returned ${parsed.length} items for ${input.length} inputs.`
    );
  }
