...
export async function translateBatchOpenAICompatible(
  input: string[],
  llm: LlmConfig,
  options: TranslateOptions
): Promise<string[]> {
  const apiKeyEnv = llm.apiKeyEnv ?? "OPENAI_COMPAT_API_KEY";
  const apiKey = process.env[apiKeyEnv];
  if (!apiKey) {
    throw new Error(`Missing OpenAI-compatible API key in env: ${apiKeyEnv}`);
  }
  ...
  if (parsed.length !== input.length) {
    throw new Error(
      `OpenAI-compatible returned ${parsed.length} items for ${input.length} inputs.`
    );
  }
