...
export async function translateBatchDeepSeek(
  ...
  if (!apiKey) {
    throw new Error(`Missing DeepSeek API key in env: ${apiKeyEnv}`);
  }
  ...
  if (!response.ok) {
    ...
    throw new Error(`DeepSeek request failed: ${response.status} ${response.statusText} - ${text}`);
  }
  ...
  if (parsed.length !== input.length) {
    throw new Error(`DeepSeek returned ${parsed.length} items for ${input.length} inputs.`);
  }
