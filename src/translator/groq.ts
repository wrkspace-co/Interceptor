...
export async function translateBatchGroq(
  ...
  const apiKeyEnv = llm.apiKeyEnv ?? "GROQ_API_KEY";
  const apiKey = process.env[apiKeyEnv];
  if (!apiKey) {
    throw new Error(`Missing Groq API key in env: ${apiKeyEnv}`);
  }

  const url = llm.baseUrl ?? "https://api.groq.com/openai/v1/chat/completions";
  ...
  if (parsed.length !== input.length) {
    throw new Error(`Groq returned ${parsed.length} items for ${input.length} inputs.`);
  }
